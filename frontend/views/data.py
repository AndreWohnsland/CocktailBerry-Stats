...
def display_data(filtered_df: pd.DataFrame, recipes_limit: int, last_day: bool) -> None:
    """Generate all the data views (plots and tables) from the data."""
    if filtered_df.empty:
        _say_no_data()
        return
    _show_filtered_size(filtered_df, last_day)
    _show_recipe_data(filtered_df, recipes_limit)
    _show_time_stats(filtered_df, last_day)
    _show_volume_stats(filtered_df)
    _show_serving_size(filtered_df)


def _show_filtered_size(filtered_df: pd.DataFrame, last_day: bool) -> None:
...
def _show_recipe_data(filtered_df: pd.DataFrame, recipes_limit: int) -> None:
...
def _show_time_stats(filtered_df: pd.DataFrame, last_day: bool) -> None:
    """Display Cocktail count over time."""
    st.header("⏱️ Data Over Time")
    hour_grouping, machine_grouping = _define_granularity(last_day)
...
def _show_volume_stats(filtered_df: pd.DataFrame) -> None:
...
def _show_serving_size(filtered_df: pd.DataFrame) -> None:
    ...
    min_servings: int = col2.slider("Filter Minimal Serving Count", 0, 10, 5)
...
def _define_granularity(last_day: bool) -> tuple[bool, bool]:
    ...
    with col2:
        selected_grouping = st.radio("Select the Time Grouping", grouping_options)
    hour_grouping = selected_grouping == grouping_options[1]
    return hour_grouping, machine_grouping


def _say_no_data() -> None:
...
def display_installations(df: pd.DataFrame) -> None:
    """Show the installation data over time and distribution."""
    st.header("📦 Installation Data")
    if df.empty:
        st.info("Currently no installation data available. Maybe it's time to install your own! ✨")
        return
